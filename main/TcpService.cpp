	...
	if (event_base == WIFI_EVENT) {
		...
		if (event_id == IP_EVENT_STA_GOT_IP || event_id == IP_EVENT_ETH_GOT_IP || event_id == IP_EVENT_PPP_GOT_IP) {
			...
			if (event != NULL) {
				...
				ESP_LOGI("DEVICE GOT IP", "IP: " "%d.%d.%d.%d",
				      IP2STR(&event->ip_info.ip));
				ESP_LOGI("DEVICE GOT IP", "MASK: " "%d.%d.%d.%d",
				      IP2STR(&event->ip_info.netmask));
				ESP_LOGI("DEVICE GOT IP", "GATEWAY: " "%d.%d.%d.%d",
...
void TcpService::ScanWifiNetworks(wifi_ap_record_t *apRecordsScanned)
{
	memset(apRecordsScanned, 0, MAXIMUM_SIZE_OF_SCAN_LIST);
	...
	ESP_ERROR_CHECK(
	      esp_wifi_scan_get_ap_records(&maximumSizeOfScanList, apRecords));
	ESP_ERROR_CHECK(esp_wifi_scan_get_ap_num(&numberOfApScanned));
	for (uint8_t i = 0; i < maximumSizeOfScanList; i++) {
	...
	memcpy(apRecordsScanned, apRecords, sizeof(apRecords));
}
